...
class PlayerCountSelectionScreen:
    """Select total target player count for LAN/online matchmaking."""

    PLAYER_COUNT_OPTIONS = [2, 3, 4]
    ...
    def __init__(self, screen: pygame.Surface, clock: pygame.time.Clock):
        ...
        self._font_number = _load_font(FONT_PATH_HEADING, 54, bold=True)
    ...
    def _build_cards(self) -> list[dict]:
        ...
        total_w = len(self.PLAYER_COUNT_OPTIONS) * card_w + (len(self.PLAYER_COUNT_OPTIONS) - 1) * gap_x
        start_x = (self.width - total_w) // 2
        y = self.height // 2 - card_h // 2 + 10

        cards: list[dict] = []
        for idx, count in enumerate(self.PLAYER_COUNT_OPTIONS):
            rect = pygame.Rect(start_x + idx * (card_w + gap_x), y, card_w, card_h)
            cards.append({"index": idx, "rect": rect, "count": count})
        return cards
    ...
    def _draw_background(self) -> None:
        if self._bg_image:
            ...
            overlay.fill((0, 0, 0, 124))
            self.screen.blit(overlay, (0, 0))
        else:
            self.screen.fill(MODE_BG_COLOR)

    def _draw_header(self) -> None:
        title = self._font_header.render("LAN PLAYER COUNT", True, (255, 255, 255))
        subtitle = self._font_body.render(
            "How many total players should this match target?",
            True,
            (205, 215, 235),
        )
        footnote = self._font_small.render(
            "Use arrow keys, Enter, or click a card",
            True,
            (170, 186, 215),
        )

        self.screen.blit(title, title.get_rect(center=(self.width // 2, 88)))
        self.screen.blit(subtitle, subtitle.get_rect(center=(self.width // 2, 130)))
        self.screen.blit(footnote, footnote.get_rect(center=(self.width // 2, 164)))
    ...
    def _draw_cards(self) -> None:
        ...
        for card in self._cards:
            ...
            if selected:
                pulse = 0.5 + 0.5 * math.sin(self._anim_time * 6.4)
                border = (98, 230, 255)
                glow = pygame.Surface((rect.width + 18, rect.height + 18), pygame.SRCALPHA)
                pygame.draw.rect(
                    glow,
                    (98, 230, 255, int(24 + 34 * pulse)),
                    glow.get_rect(),
                    border_radius=18,
                )
            ...
            count_text = self._font_number.render(str(count), True, (240, 246, 255))
            count_rect = count_text.get_rect(center=(rect.centerx, rect.centery - 14))
            self.screen.blit(count_text, count_rect)

            suffix = "PLAYERS" if count > 1 else "PLAYER"
            label = self._font_small.render(suffix, True, (190, 205, 225))
            self.screen.blit(label, label.get_rect(center=(rect.centerx, rect.bottom - 30)))
    ...
    def _move_selection(self, delta: int) -> None:
        if not self._cards:
            return
        self._selected_index = (self._selected_index + delta) % len(self._cards)
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.KEYDOWN:
                    ...
                    if event.key in (pygame.K_RETURN, pygame.K_SPACE):
                        self._fade(False)
                        return int(self.PLAYER_COUNT_OPTIONS[self._selected_index])
                    if event.key in (pygame.K_LEFT, pygame.K_a):
                        self._move_selection(-1)
                    elif event.key in (pygame.K_RIGHT, pygame.K_d):
                        self._move_selection(1)
                    elif event.key == pygame.K_2:
                        self._selected_index = self.PLAYER_COUNT_OPTIONS.index(2)
                        self._fade(False)
                        return 2
                    elif event.key == pygame.K_3:
                        self._selected_index = self.PLAYER_COUNT_OPTIONS.index(3)
                        self._fade(False)
                        return 3
                    elif event.key == pygame.K_4:
                        self._selected_index = self.PLAYER_COUNT_OPTIONS.index(4)
                        self._fade(False)
                        return 4
