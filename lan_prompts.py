...
def prompt_host_or_join(screen, clock):
    ...
    width, height = WINDOW_SIZE
    font_header = _load_font(FONT_PATH_HEADING, FONT_SIZE_HEADING + 4, bold=True)
    ...
    cards = []
    gap = 34
    total_h = len(options) * MODE_CARD_HEIGHT + (len(options) - 1) * gap
    start_y = (height - total_h) // 2 + 80

    for idx, option in enumerate(options):
        rect = pygame.Rect(
            0,
            start_y + idx * (MODE_CARD_HEIGHT + gap),
            MODE_CARD_WIDTH,
            MODE_CARD_HEIGHT,
        )
    ...
    def draw_frame(local_time: float):
        ...
        screen.blit(title, title.get_rect(center=(width // 2, int(78 + header_offset))))
        screen.blit(subtitle, subtitle.get_rect(center=(width // 2, int(136 + header_offset))))
        ...
        for idx, card in enumerate(cards):
            ...
            screen.blit(title_surf, title_surf.get_rect(midtop=(rect.centerx, rect.top + 16)))
...
def prompt_discovered_host(screen, clock):
    width, height = WINDOW_SIZE
    font_header = _load_font(FONT_PATH_HEADING, FONT_SIZE_HEADING + 2, bold=True)
    ...
    def draw_frame(local_time: float, hosts):
        ...
        screen.blit(title, title.get_rect(center=(width // 2, int(88 + header_offset))))
        screen.blit(subtitle, subtitle.get_rect(center=(width // 2, int(140 + header_offset))))

        host_rects.clear()
        list_top = int(height * 0.32)
        list_width = min(940, width - 150)
        ...
        if hosts:
            ...
            for idx, host in enumerate(hosts[:4]):
                ...
                screen.blit(title_surf, title_surf.get_rect(topleft=(rect.left + 24, rect.top + 16)))
                screen.blit(machine_surf, machine_surf.get_rect(topleft=(rect.left + 24, rect.top + 44)))
                screen.blit(address_surf, address_surf.get_rect(topright=(rect.right - 24, rect.top + 24)))
                host_rects.append((rect, host))
        else:
            panel = pygame.Rect(0, 0, min(860, width - 180), 210)
            ...
            screen.blit(waiting, waiting.get_rect(center=(panel.centerx, panel.top + 68)))
            screen.blit(helper, helper.get_rect(center=(panel.centerx, panel.top + 114)))
            screen.blit(tip, tip.get_rect(center=(panel.centerx, panel.top + 154)))
