...
class TMXTile:
    ...
    def _start_crumble(self):
        ...
        # Play crumble sound
        from audio import get_audio
        get_audio().play_sfx(SOUND_TILE_CRUMBLE, volume=0.7,
                             volume_jitter=0.10, max_instances=4)
    ...
    def update(self, dt: float):
        ...
        if self.grace_timer < self.grace_period:
            ...
            for tile in self.tiles.values():
                tile.update(dt)
            # Play countdown beep exactly when grace period ends
            if self.grace_timer >= self.grace_period:
                self.audio.play_sfx(SOUND_TILE_GRACE_END, volume=0.7, max_instances=1)
            return
        ...
        for tile in self.tiles.values():
            ...
            if just_disappeared:
                ...
                self.audio.play_sfx(SOUND_TILE_DISAPPEAR,
                                    volume=0.75, volume_jitter=0.12, max_instances=4)
    ...
    def _trigger_random_tiles(self):
        ...
        for tile in tiles_to_warn:
            ...
            self.audio.play_sfx(SOUND_TILE_WARNING,
                                volume=0.6, volume_jitter=0.10, max_instances=3)
